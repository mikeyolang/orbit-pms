...
export function brandedEmail(subject: string, text: string, content?: string) {
...
async function deliver(message: typeof mailOutbox.$inferSelect) {
  ...
  if (config.mailMode === "smtp") {
    ...
    const result = await transport.sendMail({ from: config.mailFrom ?? "Voltic PMS <notifications@orbit.local>", to: message.recipient, subject: message.subject, text: message.textBody, html: message.htmlBody ?? undefined });
...
export async function processMailOutbox(batchSize = 20) {
  const db = getDb();
  const pending = await db.select({ id: mailOutbox.id }).from(mailOutbox)
    .where(and(eq(mailOutbox.status, "pending"), lt(mailOutbox.attempts, 5)))
    .orderBy(mailOutbox.createdAt).limit(batchSize);
  let sent = 0;
  let examined = 0;
  for (const candidate of pending) {
    await db.transaction(async (tx) => {
      // A request and the background worker can flush mail simultaneously.
      // Lock each queued email so only one delivery attempt can own it.
      const [message] = await tx.select().from(mailOutbox)
        .where(and(eq(mailOutbox.id, candidate.id), eq(mailOutbox.status, "pending"), lt(mailOutbox.attempts, 5)))
        .for("update", { skipLocked: true });
      if (!message) return;
      examined++;
      try {
        const providerMessageId = await deliver(message);
        await tx.update(mailOutbox).set({ status: "sent", providerMessageId, sentAt: new Date(), attempts: message.attempts + 1, lastError: null }).where(eq(mailOutbox.id, message.id));
        sent++;
      } catch (error) {
        const attempts = message.attempts + 1;
        await tx.update(mailOutbox).set({ status: attempts >= 5 ? "failed" : "pending", attempts, lastError: error instanceof Error ? error.message.slice(0, 1000) : "Delivery failed" }).where(eq(mailOutbox.id, message.id));
      }
    });
  }
