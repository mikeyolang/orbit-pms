...
export function PermissionsPanel({ orgId, members }: { orgId: string; members: MemberLite[] }) {
  ...
  const load = useCallback(async () => {
    ...
    const [rp, mp] = await Promise.all([
      supabase.from("role_permissions").select("role, permission, allowed").eq("organization_id", orgId),
      supabase.from("member_permissions").select("user_id, permission, allowed").eq("organization_id", orgId),
    ]);
  ...
  async function resetRole(role: OrgRole) {
    const { error } = await supabase.from("role_permissions").delete().eq("organization_id", orgId).eq("role", role);
                ...
                {PERMISSION_GROUPS.map((group) => (
                  <Fragment key={group}>
                    <tr className="border-t border-border/60 bg-muted/20">
                      <td colSpan={ASSIGNABLE_ROLES.length + 1} className="px-4 py-1.5 text-[11px] uppercase tracking-wider text-muted-foreground">
                        {group}
                      </td>
                    </tr>
          ...
          {currentMember ? (
            <div className="mt-4 space-y-1.5">
              {PERMISSIONS.map((p) => {
                ...
                const inherited = roleValue(currentMember.role, p.key);
                return (
                  <div key={p.key} className="flex items-center gap-3 rounded-lg border border-border/60 bg-background p-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Role default ({currentMember.role}): {inherited ? "allowed" : "blocked"}
                      </div>
                    </div>
                    <Select
                      value={override}
