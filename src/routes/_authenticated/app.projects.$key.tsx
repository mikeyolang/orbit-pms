  ...
  return (
    <div className="mx-auto max-w-7xl px-8 py-6">
      ...
      <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-6">
        {stats.map((s) => (
          <div key={s.value} className="rounded-lg border border-border/60 bg-card/30 px-3 py-2">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{s.label}</div>
            <div className="mt-0.5 text-lg font-semibold">{s.count}</div>
          </div>
        ))}
      </div>
      ...
      <Tabs defaultValue="board" className="mt-6">
        ...
        <TabsContent value="sprints" className="mt-4">
          <SprintsTab projectId={project.id} sprints={sprints} onChanged={load} tasks={rootTasks} projectKey={project.key} />
        </TabsContent>

        <TabsContent value="milestones" className="mt-4">
          <MilestonesTab projectId={project.id} milestones={milestones} onChanged={load} tasks={rootTasks} projectKey={project.key} />
        </TabsContent>
      </Tabs>
...
// ---------- Sprints ----------
function SprintsTab({ projectId, sprints, onChanged, tasks, projectKey }: {
  projectId: string; sprints: Sprint[]; onChanged: () => void; tasks: Task[]; projectKey: string;
}) {
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <NewSprintDialog projectId={projectId} onCreated={onChanged} />
      </div>
...
function NewSprintDialog({ projectId, onCreated }: { projectId: string; onCreated: () => void }) {
...
// ---------- Milestones ----------
function MilestonesTab({ projectId, milestones, onChanged, tasks, projectKey }: {
  projectId: string; milestones: Milestone[]; onChanged: () => void; tasks: Task[]; projectKey: string;
}) {
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
      ...
      {milestones.map((m) => {
        const items = tasks.filter((t) => t.milestone_id === m.id);
        const done = items.filter((t) => t.status === "done").length;
