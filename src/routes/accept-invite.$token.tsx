...
function AcceptInvite() {
  ...
  const respond = useCallback(async (action: "accept" | "decline") => {
    ...
    try {
      ...
      if (!response.ok) return setError(body.error ?? "Unable to respond to invitation");
      if (action === "accept") { setCurrentOrgId(body.organizationId); navigate({ to: body.teamId ? "/app/teams" : "/app", replace: true }); }
      else setInvite((current: any) => ({ ...current, declined_at: new Date().toISOString() }));
  ...
  useEffect(() => {
    ...
    }).catch(() => setError("Unable to load this invitation. Please refresh to try again."));
  ...
  return <AuthShell title={`Join ${invite.organization_name}`} subtitle={invite.team_name ? `Join the ${invite.team_name} team in this workspace.` : `You have been invited as ${invite.role_name ?? invite.role}`}>
    <div className="space-y-3">
      ...
      {signedIn ? <><Button className="w-full" disabled={working} onClick={() => respond("accept")}>{invite.team_name ? "Accept and join team" : "Accept and open workspace"}</Button><Button className="w-full" variant="outline" disabled={working} onClick={() => respond("decline")}>Decline invitation</Button></> : invite.has_account ? <Button className="w-full" onClick={() => continueToAuth("signin")}>Sign in to accept</Button> : <><Button className="w-full" onClick={() => continueToAuth("signup")}>Create account and join</Button><p className="text-center text-xs text-muted-foreground">After verifying your email, you’ll be added and taken directly to the workspace.</p></>}
    </div>
  </AuthShell>;
}
