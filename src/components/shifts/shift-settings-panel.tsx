...
export function ShiftSettingsPanel({ orgId, canManage }: { orgId: string; canManage: boolean }) {
  ...
  async function updateSetting(patch: Partial<ShiftSettings>) {
    if (!settings) return;
    const { error } = await supabase.from("shift_settings").update(patch).eq("id", settings.id);
    if (error) return toast.error(error.message);
    setSettings({ ...settings, ...patch });
  }

  async function updateType(id: string, patch: Partial<ShiftType>) {
    const { error } = await supabase.from("shift_types").update(patch).eq("id", id);
    if (error) return toast.error(error.message);
